...
def main():
	kernel_v = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype=np.float)
	kernel_h = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=np.float)
	...
	cv2.imwrite("./result_15_v.jpg", gray_img_filtered_v)
	cv2.imwrite("./result_15_h.jpg", gray_img_filtered_h)
