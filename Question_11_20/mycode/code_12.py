...
def main():
	...
	kernel = np.diag([1] * ksize).astype(np.float)
	kernel /= ksize
	...
	for i in range(img.shape[0]):
		for j in range(img.shape[1]):
			for c in range(img.shape[2]):
				img_filtered[i+padsize, j+padsize, c] = np.sum(kernel * img_pad[i:i+ksize, j:j+ksize, c]).astype(np.uint8)

	img_filtered = img_filtered[padsize:padsize+img.shape[0], padsize:padsize+img.shape[1], :]
	cv2.imwrite("./result_12_{}.jpg".format(ksize), img_filtered)
